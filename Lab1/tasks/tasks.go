...
func Task1() {
	var input string // Переменная для пользовательского ввода
	for {
		...
		fmt.Println("\nВведите 'q' для выхода или нажмите Enter для продолжения...")
		fmt.Scanln(&input) // Считываем ввод

		if input == "q" { // Проверяем, хочет ли пользователь выйти
